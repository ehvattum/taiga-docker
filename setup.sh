...
sudo docker run -d -e VIRTUAL_HOST=taiga.ragesheep.com -p 5432 --name postgres -v /data/postgresql:/var/lib/postgresql/data postgres
sudo docker run -d -e VIRTUAL_PORT=80 -e VIRTUAL_HOST=taiga.ragesheep.com -p 8000 -p 80 --name taiga-front --link taiga-back:taiga-back ipedrazas/taiga-front
sudo docker run -d -e VIRTUAL_HOST=taiga.ragesheep.com -p 8001 --name taiga-back --link postgres:postgres ipedrazas/taiga-back
